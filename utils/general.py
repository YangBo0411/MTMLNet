...
LOGGING_NAME = "multitask"  #yb 2024.03.21
...
def print_mutation(keys, results, seg_results, hyp, save_dir, bucket, prefix=colorstr('evolve: ')):
    evolve_csv = save_dir / 'evolve.csv'
    evolve_yaml = save_dir / 'hyp_evolve.yaml'
    keys = tuple(keys) + tuple(hyp.keys())   # 源码
    # keys = tuple(keys) + tuple(hyp.keys()) + tuple(seg_results.keys())  # [results + hyps] # yb 2024.03.25 添加 seg_results
    keys = tuple(x.strip() for x in keys)
    vals = results + tuple(hyp.values())       #源码
    # vals = results + tuple(hyp.values()) + tuple(seg_results.values())      #yb 2024.03.25 添加 seg_results
    n = len(keys)
